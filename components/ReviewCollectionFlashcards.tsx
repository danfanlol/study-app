...
type ReviewCollectionFlashcardsProps = {
  setIds: string[]
}

export default function ReviewCollectionFlashcards({ setIds }: ReviewCollectionFlashcardsProps) {
  ...
  function handleCorrect() {
    ...
    setShowBack(false)
    if (updatedQueue.length === 0) setReviewComplete(true)
  }

  function handleWrong() {
    if (reviewQueue.length === 0) return
    const currentCard = reviewQueue[0]
    setReviewQueue([...reviewQueue.slice(1), currentCard])
    setShowBack(false)
  }
  ...
  return (
    <div>
      <h2 className="mb-4 text-2xl font-semibold">Review Collection</h2>
      ...
      {userLoading || loading ? (
        ...
        <p>No flashcards found across the sets in this collection.</p>
      ) : reviewComplete ? (
